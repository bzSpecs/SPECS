...
highlight_minimum_value = int(sys.argv[5])
...
merged_by_barcode = merged_by_barcode.astype({'count': 'int64'})

merged_by_promoter = merged_by_barcode.groupby(
    ['promoter']).sum().reset_index().sort_values(by=['count'], ascending=False)

merged_by_barcode = merged_by_barcode[~(
    merged_by_barcode['count'] <= highlight_minimum_value)]

merged_by_promoter = merged_by_promoter[~(
    merged_by_promoter['count'] <= highlight_minimum_value)]
